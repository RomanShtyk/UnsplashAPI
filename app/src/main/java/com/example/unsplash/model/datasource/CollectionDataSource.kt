...
class CollectionDataSource : PageKeyedDataSource<Int, ColletionPhotos>() {
    ...
    override fun loadInitial(params: LoadInitialParams<Int>, callback: LoadInitialCallback<Int, ColletionPhotos>) {
        unsplashAPI.getCollections(FIRST_PAGE).enqueue(object : Callback<List<ColletionPhotos>> {
            override fun onResponse(call: Call<List<ColletionPhotos>>, response: Response<List<ColletionPhotos>>) {
